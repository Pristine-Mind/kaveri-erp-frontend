...
const Products: React.FC = () => {
  ...
  const mainCatgegoryOptions: Category[] = [
    { value: 'BV', label: t('Beverage') },
  ]

  const categoryOptions: Category[] = [
    { value: 'SB', label: t('strong_beer') },
    { value: 'PB', label: t('premium_beer') },
    { value: 'SE', label: t('special_edition') },
  ...
  useEffect(() => {
    ...
    fetchProducers();
    setmainCategories(mainCatgegoryOptions);
                    ...
                    {showProducerList && (
                      <ul className="absolute z-10 bg-white border rounded-lg w-full max-h-48 overflow-y-auto mt-1">
                        ...
                        {suppliers.filter((supplier) =>
                          ...
                          <li className="px-4 py-2">{t('no_supplier_found')}</li>
                        )}
                    </ul>
                  )}
